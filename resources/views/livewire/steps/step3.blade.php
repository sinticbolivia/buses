<div class="w-full max-w-7xl mx-auto px-4">
    <div class="grid grid-cols-3 gap-4">
        <div class="flex flex-col space-y-4">
            ...
            <div id="asientos" class="border px-2 py-3 rounded-lg">
                @foreach ( $asientos as $fila)
                    <div class="grid gap-y-4 my-2 grid-cols-{{ $propiedades['filas'] }}">
                        @foreach ( $fila as $asiento )
                            ...
                            @else
                            <div class="bg-gray-200 rounded-md flex items-center justify-center font-bold py-2 select-none cursor-pointer text-xs {{ $asiento['ocupado'] ? 'bg-red-200' : '' }} {{ !$asiento['ocupado'] ? 'hover:bg-red-300' : '' }} {{ $asiento['reservado'] ? 'bg-green-300' : '' }} {{ $asiento['gradas'] ? 'bg-gray-400 text-gray-500': '' }}"
            ...
            @if ( $show )
                <div class="bg-white px-6 py-5 rounded-lg shadow-lg">
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                        <div>
                            ...
                            <select id="tipo" onchange="reservas()" wire:click="seleccionarTipo({{ $tipo }})" name="tipo" wire:model="tipo" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-yellow-200 focus:border-yellow-200 sm:text-sm">
                        ...
                        <div>
                            ...
                            <input  readonly wire:model="numero_asiento" type="text" name="numero_asiento" id="numero_asiento" class="mt-1 focus:ring-yellow-200 focus:border-yellow-200 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                        ...
                        <div>                          
                            ...
                            @if($tipo == 'venta')
                                <input x-model="precio" readonly wire:model="precio" type="number"  name="precio" id="precio" class="mt-1 focus:ring-yellow-200 focus:border-yellow-200 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                            @endif

                            @if($tipo == 'reserva')                        
                                <input x-model="precio" type="number" min="0" name="precio" value="0" class="mt-1 focus:ring-yellow-200 focus:border-yellow-200 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">      
                            @endif
                        ...
                        <div>
                            ...
                            <input type="text" name="ci" id="ci" wire:model="ci" wire:model="customer.identification" wire:keydown.tab="searchCi" wire:keydown.enter="searchCi" class="ci mt-1 focus:ring-yellow-200 focus:border-yellow-200 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" required>
                            <x-jet-input-error for="ci" class="mt-2" />
                        </div>
                                                            
                        <div class="col-span-2">
                            <label for="nombres" class="block text-sm font-medium text-gray-700"> Nombres</label>
                            <input type="text" name="nombres" id="nombres" wire:model="nombres" wire:model="customer.name" required class="mt-1 focus:ring-yellow-200 focus:border-yellow-200 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                            <x-jet-input-error for="nombres" class="mt-2" />
                        </div>
                        <div class="col-span-2">
                            <label for="apellidos" class="block text-sm font-medium text-gray-700">Apellidos</label>
                            <input type="text" name="apellidos" id="apellidos" wire:model="apellidos" wire:model="customer.lastname" value="" required class="mt-1 focus:ring-yellow-200 focus:border-yellow-200 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                            <x-jet-input-error for="apellidos" class="mt-2" />
                        </div>
                        --}}
                        <div>
                            <label for="ci" class="block text-sm font-medium text-gray-700">CI / NIT</label>
                            <input type="text"  name="ci" id="ci" wire:model="customer.identification" wire:keydown.tab="searchCi" wire:keydown.enter="searchCi" class="ci mt-1 focus:ring-yellow-200 focus:border-yellow-200 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" required>
                            <x-jet-input-error for="ci" class="mt-2" />
                        </div>

                        <div class="col-span-2">
                            <label for="nombres" class="block text-sm font-medium text-gray-700"> Nombres</label>
                            <input type="text"  name="nombres" wire:model="customer.name"  class="mt-1 focus:ring-yellow-200 focus:border-yellow-200 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                            <x-jet-input-error for="nombres" class="mt-2" />
                        </div>
                        <div class="col-span-2">
                            <label for="apellidos" class="block text-sm font-medium text-gray-700">Apellidos</label>
                            <input type="text"  name="apellidos"  wire:model="customer.lastname" value=""  class="mt-1 focus:ring-yellow-200 focus:border-yellow-200 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                            <x-jet-input-error for="apellidos" class="mt-2" />
                        </div>
                    ...
                    <div class="grid grid-cols-4">
                        ...
                        <div class="w-full mt-4 ">
                            <button wire:click="storeBoleto" type="button" 
                                class="py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-yellow-500 hover:bg-yellow-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-yelow-400 flex items-center">
                        ...
                        @if($tipo == 'venta')
                        <div class="w-full mt-4 ">
                            <button wire:click="libelula" type="button"
                                class="py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-yellow-500 hover:bg-yellow-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-yelow-400 flex items-center">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                Siguiente
                            </button>
                ...
                <div class="py-12 px-10">
                    <div class="py-3">Asientos Ocupados
                        <button class="py-2 px-4 bg-red-300 box-border h-10 w-10 border-4">
                            </button>
                    </div>
                    <div class="py-3">Asientos Reservados
                        <button class="bg-green-300 py-3 px-4 box-border h-10 w-10 p-4 border-4">
                        </button>
                    </div>
                    <div class="py-3">Asientos Libres
                        <button class="bg-gray-200 py-3 px-4 box-border h-10 w-10 p-4 border-4">
                        </button>
                    </div>
                </div>
            @endif
...
<script>
    function reservas(){   
        ...
        if($res=='reserva'){
            ...
            document.getElementById('precio').setAttribute('readonly');
        }
    }
</script>
